const AVATARS = ['🧑‍🦲', '👩‍🦰', '👩', '🧕', '👩🏻‍🦳', '👨🏽', '🧑‍🦱', '👱', '🧔', '🧑‍🦰'];
const MARKERS = [
  { id: 0, color: '#d63031', cost: 0, name: "Рубин" },
  { id: 1, color: '#00b894', cost: 15000, name: "Изумруд" },
  { id: 2, color: '#0984e3', cost: 15000, name: "Сапфир" },
  { id: 3, color: '#fdcb6e', cost: 15000, name: "Янтарь" }
];
const DAILY_REWARDS = [
...
function speakNumber(num) {
  if ('speechSynthesis' in window) {
    ...
    const nicknameText = NICKNAMES[num] ? `. ${NICKNAMES[num]}!` : '';
    const utterance = new SpeechSynthesisUtterance(`${num}${nicknameText}`);
...
async function loadUser() {
  ...
  if (user.isBanned) {
    showGiftNotifications({ from: "СИСТЕМА БЕЗОПАСНОСТИ", bills: 0, coins: 0, msg: "Ваш профиль заблокирован!" });
...
function renderShop() {
  ...
  SHOP_ITEMS.forEach(item => {
    ...
    row.innerHTML = `<span>${item.bills} 💵</span><button class="btn-glow">${item.coins} 🪙</button>`;
...
function renderMarkers() {
  ...
  MARKERS.forEach(m => {
    ...
    const cell = document.createElement('div');
    cell.className = 'marker-color-palette-node';
    cell.style.borderLeft = `8px solid ${m.color}`;
    cell.innerHTML = `<div><b>${m.name}</b></div><button class="btn-glow" style="font-size:12px; padding:6px 12px;">${owned ? (user.marker === m.id ? 'Выбран' : 'Взять') : m.cost + ' 💵'}</button>`;
...
function startGame(mode, numCards) {
  ...
  gameState.timer = setInterval(() => {
    ...
    drum.offsetHeight; 
    drum.style.animation = 'barrelRollIn 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards';
...
function renderTickets() {
  ...
  container.innerHTML = '';
  const markerColor = MARKERS.find(m => m.id === user.marker)?.color || '#d63031';
      ...
      row.forEach((val, ci) => {
        ...
        if (val === null) {
          ...
          if (gameState.marks[ti][ri][ci]) {
            cell.classList.add('chip-marked');
            cell.style.background = markerColor;
          } else if (gameState.drawn.includes(val)) {
            cell.classList.add('missed-number-alert');
          }
          cell.addEventListener('click', () => {
...
document.getElementById('btn-open-multiplayer').addEventListener('click', () => openModal('multiplayer'));
...
async function syncRoom() {
  ...
  document.getElementById('multi-players-list').innerHTML = room.players.map(p => `<div class="wallet-item" style="color:#fff; margin-bottom:5px;">🧑‍💻 ${p.name}</div>`).join('');
...
async function updateGlobalChat() {
  ...
  chatBox.innerHTML = messages.map(m => `<div style="margin-bottom:8px; text-align:left;"><b>${m.name}</b>: ${m.text}</div>`).join('');
  chatBox.scrollTop = chatBox.scrollHeight;
}
setInterval(updateGlobalChat, 2000);

document.getElementById('btn-admin-login').addEventListener('click', () => openModal('admin'));
...
async function refreshAdminPlayers() {
  ...
  document.getElementById('admin-players-list').innerHTML = players.map(p => `<div style="background:rgba(0,0,0,0.3); padding:10px; margin-top:5px; border-radius:10px; text-align:left;"><div><b>${p.name}</b> ${p.isBanned ? '[БАН]' : ''}</div><div>💵 ${p.bills}</div><button onclick="togglePlayerBan('${p.uid}', 'ban')" class="btn-exit" style="background:#c0392b;">Бан</button></div>`).join('');
}

window.togglePlayerBan = async (targetUid, banAction) => {
  await fetch('/api/admin/ban', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminPassword: currentAdminPassword, targetUid, banAction }) });
  refreshAdminPlayers();
};

function showGiftNotifications(gift) {
  document.getElementById('gift-alert-text').textContent = gift.msg || "Бонус зачислен!";
  openModal('gift-alert');
}
document.getElementById('btn-close-gift-alert').onclick = () => document.getElementById('modal-gift-alert').classList.add('hidden');
